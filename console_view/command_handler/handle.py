...
class CommandHandler:
    def __init__(
        self, database_view: AccountBaseView, transaction_view: TransactionBaseView, viewer: Viewer, bank: Bank
    ) -> None:
            ...
            case "add", *args:
                match args:
                    ...
                    case "transfer", str(from_account), str(to_account), str(value), str(currency):
                        ...
                        self.__transaction_view.add_transaction(transaction)
                        transaction.connect(
                            self.__database_view.get_account(from_account), self.__database_view.get_account(to_account)
                        )
                    case _:
