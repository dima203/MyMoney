...
class TestJSONBaseAdd:
    ...
    def test_add_first_record(self):
        pk = self.db.add({"name": "test", "value": 10})
        assert pk == 1
        data = self.db.load()
        assert len(data) == 1
        assert data[0]["id"] == 1
    ...
    def test_add_multiple_records(self):
        self.db.add({"name": "first"})
        pk2 = self.db.add({"name": "second"})
        assert pk2 == 2
        data = self.db.load()
        assert len(data) == 2

    def test_add_with_explicit_pk(self):
        pk = self.db.add({"id": 100, "name": "explicit"})
        assert pk == 100
        data = self.db.load()
...
class TestJSONBaseUpdate:
    def setup_method(self) -> None:
        ...
        self.db = JSONBase(str(self.path))
        self.db.add({"id": 1, "name": "original", "value": 10})
...
class TestJSONBaseDelete:
    def setup_method(self) -> None:
        ...
        self.db = JSONBase(str(self.path))
        self.db.add({"id": 1, "name": "first"})
        self.db.add({"id": 2, "name": "second"})
